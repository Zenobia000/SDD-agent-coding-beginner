...
先固定 diff 或明確 scope，再委派 `.agents/agents/security-reviewer/agent.md` 做獨立只讀審查（⚠️ Antigravity workspace subagent 格式官方未載明；無法載入時改為在乾淨 context 以同一份輸入自行審查，並保持只讀）。提供：
