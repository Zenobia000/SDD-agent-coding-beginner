...
Standards 與 Spec 必須由彼此隔離 context 的 subagent 分別審查，再並列呈現；不要讓其中一軸替另一軸洗掉問題。
...
委派給 `.agents/agents/` 的兩個 subagent，各自使用獨立 context：
...
兩者不能看到對方輸出，也不能修改檔案。若變更涉及認證、授權、付款、上傳、秘密、外部 API 或不可逆資料變更，再獨立委派 `security-reviewer`，但不要把 Security 合併進前兩軸。

⚠️ Antigravity workspace subagent 的檔案格式官方文件未載明。若 `.agents/agents/` 的定義無法載入，改為分兩次獨立執行兩軸審查：第二軸開始前不得讀取第一軸的結論，且回報時仍分軸並列。
