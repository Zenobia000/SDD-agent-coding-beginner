...
5. 術語一旦收斂立即更新 glossary，使用 [CONTEXT-FORMAT.md](./references/CONTEXT-FORMAT.md)。glossary 只放領域語言，不放 implementation、spec 或 session notes。
6. 只有同時符合「難逆轉、缺脈絡會令人意外、存在真實取捨」才提議 ADR；使用 [ADR-FORMAT.md](./references/ADR-FORMAT.md)，取得同意後才寫。
