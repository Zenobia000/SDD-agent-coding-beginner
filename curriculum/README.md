...
第 1 章不能跳。它產生的 `docs/agents/project.md` 是第 2、3 章的第一個輸入（`grill-with-docs`、`to-spec` 都先讀它）；跳過會讓學生從第 2 章就得自己回答 AI 問的專案設定問題。時間真的不夠，就由講師預先備妥該檔發給學生，不要讓 skill 落到自行探索的 fallback。
