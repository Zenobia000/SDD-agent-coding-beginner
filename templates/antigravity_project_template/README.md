...
└── .agents/
    ...
    │   ├── test.md                ← /test
    │   ├── explain.md             ← /explain
    │   ├── check-key.md           ← /check-key
    │   ├── git/commit.md          ← /git:commit
    │   ├── vibe/plan.md           ← /vibe:plan
    │   └── explain-code/          ← /explain-code（進階：資料夾形式）
...
| 清空對話重新開始          | 打 `/clear`                                |
| 看內建 slash 指令       | 打 `/help`                                 |
| 觸發自訂 skill        | 打 `/test`、`/explain`、`/git:commit`、`/vibe:plan` 等 |
