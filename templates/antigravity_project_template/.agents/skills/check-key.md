...
| `/check-key`（本 skill） | **部署 / push public 前**（最後一道） | + 未填 placeholder + .gitignore 覆蓋 + 環境變數設定 + 已 commit 過的 secret rotate 建議 |

**心法**：`/verify` 是「日常每次 commit 的快檢」、`/check-key` 是「部署前的全身體檢」。學員看到自己 push 到 GitHub 那一刻 = 必跑這個。
