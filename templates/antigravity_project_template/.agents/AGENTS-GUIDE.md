# 怎麼寫好 AGENTS.md — 研究實證版

> 這份是「**怎麼寫**站立規則」的方法論；本專案實際的站立規則在根目錄 [`AGENTS.md`](../AGENTS.md)。
> 想擴充能力先讀 [`SKILLS.md`](./SKILLS.md) / [`MCP.md`](./MCP.md) / [`SUBAGENTS.md`](./SUBAGENTS.md)，這份是回頭優化主檔時看的。

---

## 一句話講白

**AGENTS.md = 為一個「每次失憶、但完全信任文件」的 agent 做記憶植入。**

它是業界開放標準（Anthropic / Google / Microsoft / OpenAI 共同捐給 Linux Foundation 的 Agentic AI Foundation，約 6 萬個 repo 在用）。Antigravity / Claude Code / Cursor / Codex / Copilot 都讀它，所以**寫一份、跨工具通用**。
...
這是整份方法論的地基。Agent 會讀 `package.json`、`pyproject.toml`、目錄結構、README——這些它自己看得到的，寫進 AGENTS.md 是**純浪費上下文**。
...
> **自動 `/init` 生出來不修的 AGENTS.md，平均降低 3% 成功率、增加 20% 推論成本。**
...
**作者澄清（Thibaud Gloaguen）**：論文反對的是「LLM 自動產生」的 AGENTS.md，**不是反對這個檔案**。核心提問：
...
> 「絕對不能 commit secret」寫進 AGENTS.md，有 30% 機率被無視。
...
> - 規則攔的是 **agent 工具生命週期**（某類工具呼叫前先檢查）→ **agent-settings hook**（`.agents/settings.json` 的 `before_tool_call` 等 / Claude `.claude/settings.json`）。只攔 agent，攔不到你手敲的指令。
>
...
| 層 | 放什麼 | 在本模板對應 |
|---|---|---|
| **根 `AGENTS.md`** | 技術棧、專案身分、路標（指向細節在哪） | [`../AGENTS.md`](../AGENTS.md) |
| **子目錄 `AGENTS.md`** | 該領域特定指引（monorepo 各 package） | 視專案需要新增 |
| **`.agents/` 拆主題** | 規則 / 工作流 / skill 分檔 | [`rules/`](./rules/) `WORKFLOW.md` `skills/` |
...
| `.cursorrules` | Cursor | symlink → AGENTS.md |
| `.claude/settings.json` | Claude Code | hooks（機械層強制，非文件） |

**多工具團隊**：寫一份 `AGENTS.md`，其餘用 symlink 指過去，別維護平行版本。
...
## 套用回本模板:我們的 AGENTS.md 自評

對照上面準則，誠實打分本模板的 [`../AGENTS.md`](../AGENTS.md)：
...
**為什麼不是 `settings.json` hook**：查證後（[Antigravity Hooks 文件](https://antigravity.google/docs/hooks)）其 hook 點是 agent 工具生命週期，攔不到「人手敲 `git commit`」。secret/force-push 的威脅是 git 操作，**git hook 才是對的層**，且跨工具通用。日常仍搭 [`/check-key`](./skills/check-key.md)（agent 判斷力）做部署前體檢。

---

## 速查清單（寫 / 改 AGENTS.md 前過一遍）
...
**資料來源**：本文濃縮自 [blog.aihao.tw《AGENTS.md 研究與實踐》](https://blog.aihao.tw/2026/05/03/agents-md-research-and-practices/)，
