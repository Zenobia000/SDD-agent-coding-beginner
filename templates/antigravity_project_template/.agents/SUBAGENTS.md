# Subagents 入門 — Antigravity CLI 平台特性

> Subagents 是 Antigravity CLI 2026 新增的**平行任務原語**。
> 跟 [MCP](./MCP.md) / [Skills](./SKILLS.md) 並列為「平台特有、模型本能彌補不了」的三大擴充原語。
> 不會用基本對話之前**先跳過這份**，把 AGENTS.md + Skills 用熟再回來。
...
過去 Gemini CLI 時代，一個 agent = 一個對話 = 一條線跑到底，遇到大型任務 context 會爆、跑很久。Antigravity 2026 內建多代理調度（**無須 tmux**），主 agent 可以動態派 N 個 subagent 並行處理。
...
| **檔案** | `settings.json` 內 `mcpServers` | `.agents/skills/<name>.md` | 對話中宣告 / Skill body 內宣告 |
...
| 工具 | Subagent 支援度 |
|---|---|
| **Antigravity CLI** | ✅ 原生支援，無須額外配置（2026 殺手特性） |
...
**Antigravity 的優勢**：subagent 是平台原語，不是 hack。主 agent 派 / 收 / 彙整全自動，使用者只管講「請用 subagents 處理 X」。
...
- [Antigravity Dynamic Subagents 官方文件](https://antigravity.google/docs/subagents)
- [Antigravity CLI Deep Dive — Subagents 段（agentpedia）](https://agentpedia.codes/blog/antigravity-cli-deep-dive)
