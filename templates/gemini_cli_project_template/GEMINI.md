# GEMINI.md — 給 Gemini CLI 的總指揮文件

> Gemini CLI 啟動時會自動讀這個檔案。這是「站立規則」，每次對話都生效。
> 如果你想確認 Gemini 真的有讀到，在 CLI 內打 `/memory show`。
...
2. `.gemini/rules/01-keep-it-simple.md` — 簡單第一原則
3. `.gemini/rules/02-coding-style.md` — code 風格
4. `.gemini/rules/03-when-stuck.md` — 卡關 SOP

**要呼叫 MCP 工具前**，先看 `.gemini/MCP.md` 該工具的安全警告。
...
- `.gemini/settings.json` 內 `mcpServers` 是擴充工具清單，可呼叫 `/mcp` 看當前狀態
- 詳細用法與安全警告見 `.gemini/MCP.md`
