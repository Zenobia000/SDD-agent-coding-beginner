...
> **為什麼有這個資料夾**：CLAUDE.md §7 寫了「不准 commit secret、不准 force-push」，
> 但**寫進文件只有 ~70% 順從率**。不可逆的操作不能賭那 30%，所以改用機械層強制。
> 方法論見 [`docs/authoring/04-write-a-hook.md`](../../../docs/authoring/04-write-a-hook.md)。
> 真正不可破的規則要放在**機械層**——這就是這些 git hook 的工作。

## 為什麼是 git hook，不是 Claude Code settings.json hook

| 層 | 攔什麼 | 攔得到誰 |
|---|---|---|
| **git hook（這裡）** | git 操作（commit / push） | **人 + 任何 AI agent，跨工具一律生效** |
| Claude Code `settings.json` hook | agent 的工具呼叫生命週期 | 只攔 agent，攔不到你手敲的 `git commit` |
...
> 「忘了填金鑰 placeholder」不在這擋——那是功能 bug 不是洩漏，交給 `/sec-scan`（agent 有判斷力，不會誤擋 `.env.example` 或文件本身）。hook 只守「一旦發生就不可逆」的安全威脅。
