...
> 真實 sprint 結尾跑 `/ship` skill，AI 會引導你寫一份新的。
...
- **AI 跑 /tdd 時主動先問前置條件**，避免我直接寫 code 跑掉
- **BDD scenarios 寫完後，AI 自動生對應 unit test 骨架**很省事
- **`/review` 一鍵跑完 5 個維度**，比手動跑舒服很多
- **每天結束前打 `/review` 確認沒漂移**變成自然習慣
- **Conventional commits 配 `/ship`**，git log 變得有用了

### 🎓 Learned（學到的）

- **TDD 紅綠燈不是工作量加倍** — 是把「事後 debug 5 小時」前置成「事前寫測試 30 分鐘」
- **ADR 寫一次後**，再次討論「要不要用 X」AI 自動 reference 過去決策，省了重新思考
- **CLAUDE.md 寫角色 + 必讀文件 + 絕對禁止** 這三段最有用，其他章節 AI 自動會做
...
- **第一天太想一次寫完，沒走 /spec** → 後續花更多時間補

### 💭 Longed for（希望未來有什麼）

- 想要一個 `/risk-check` skill 在 sprint 中途快速掃 risk
- 想要 `/spec` 自動把「retry / timeout / fallback」三件事問清楚
...
1. [ ] 在 `.claude/skills/spec/templates/PRD-template.md` 加入「Failure & Retry Policy」必填欄
