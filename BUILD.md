...
> **真的卡住的時候**：[`curriculum/COOKBOOK.md`](./curriculum/COOKBOOK.md) 有每一站的指令與逐字可貼的提示詞。它是逃生口不是主線 —— **先自己打一次，卡住超過 20 分鐘再翻**。代價寫在[附錄](#附錄這本書刻意沒有給你的東西)。
...
### 可以直接貼的 prompt：在 COOKBOOK，但先自己試一次

這本書每一站只寫一行指令，是刻意的 —— 技能本身就是 prompt，你要學的是**選哪一個**。

但「刻意留白」和「卡在原地兩小時」是兩件事，這兩件事以前被混在一起了。所以有 [`curriculum/COOKBOOK.md`](./curriculum/COOKBOOK.md) —— 七站每一站的指令、逐字可貼的提示詞、離站前的自我檢查。

**它是逃生口，不是主線。** 翻之前先看你要付什麼：

| 你提早翻了 | 你會失去 |
|---|---|
| 第 1 站的四個問題 | 「自己想出該問什麼」的那次練習。而那是整條路線最重要的一站 |
| 第 3 站的「跑 fixture 再判斷」 | 用 `caption-only` 那支撞出「架構決定要靠實測」的那一刻 |
| 第 4 站的「垂直切片」 | 把票切成分層、做到一半什麼都不會動的那次痛 |
| 第 7 站的 prompt injection | 先試再修的順序 —— 看自己的系統當場中招的那個震撼 |

建議：**每一站先照這本書自己打一次，真的卡住超過 20 分鐘再去翻。** 翻完把你原本打的和範本對照一次 —— 差異在哪裡，那就是你這一站真正學到的東西。

COOKBOOK **不會**給你任何一個決定的答案。schema 怎麼設計、選 VLM 還是 STT、Non-goal 寫什麼 —— 裡面每個提示詞都是把問題丟回給你的形狀。它幫你省掉打字，不幫你省掉判斷。
