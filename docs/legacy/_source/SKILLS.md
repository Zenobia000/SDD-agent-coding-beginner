# Skills（Agent Skills）入門 — Claude Code 版

> Skill 是 Claude Code 的核心擴充原語：放在 `.claude/skills/` 的 markdown 檔，
> 既是 AI 可以「自動觸發」的程序性知識，也是你可以「手動打 `/xxx` 」的 slash command。
> 不會用基本對話之前**先跳過這份**，把 CLAUDE.md + MCP 用熟再回來。

---

## 一句話講白

**Skill = 把「程序性知識」封裝成 markdown 檔案 + 資料夾，Claude Code 啟動時掃描載入。**

舉例：你每次寫完 code 都會手動講「請檢查命名一致性、檢查有沒有硬編碼、列出改了哪些檔案」——這個審查流程寫成 Skill 後，AI 看你說「我寫完了」會**自動翻到這份食譜**照做。

**重要變化（vs Gemini CLI）**：
Claude Code 把舊版 Gemini CLI 的 `commands/.toml` 與 `skills/SKILL.md` **合併成單一原語**。同一個 markdown 檔，AI 看 description 會自動觸發，使用者也能手動打 `/<檔名>`。一個 Skill 兩種觸發方式。

---

## Skill 在三大原語裡的定位

Skill / MCP / Subagent 是 Claude Code 的三大擴充原語。**完整對照表與判斷練習見 [`SUBCLAUDE.md` §三大擴充原語對照](./SUBCLAUDE.md#三大擴充原語對照mcp--skill--subagent)**。
...
Claude Code 啟動時會掃描以下位置：

| 來源 | 路徑 | 適合放 |
|---|---|---|
| **User**（全域） | `~/.gemini/antigravity-cli/skills/` | 你個人習慣的審查 / 思考流程 |
| **Workspace**（專案） | `<project>/.claude/skills/` | 專案專屬的流程，**會跟 git 走** |

> 過渡期說明：Claude Code 的全域目錄仍然落在 `~/.gemini/antigravity-cli/`（Google 把 Gemini CLI 的舊家底沿用過來方便 `agy plugin import gemini` 一鍵搬遷）。專案目錄則統一用 `.claude/`（對齊 `CLAUDE.md` 跨工具共通格式）。

**初學者只要管 Workspace skill**：放在 `.claude/skills/` 下，跟著專案進 git，未來團隊或下個專案都能複用。
...
打開 `.claude/skills/explain-code/SKILL.md`，第一段寫：
...
**`description` 是觸發關鍵**——Claude Code 會用它跟你的問題做語意比對。三個秘訣：
...
## description 怎麼寫得讓 Claude Code 找得到
...
本模板內建 **10 個 skill**（2 個 循環工程 基礎 + 8 個 循環工程 Sprint 工作流）。為避免重複，完整列表、分類、觸發時機、對標大廠規範與工作流順序統一在以下兩份文件，本檔不重述：
...
下面是「常見但非必要」的 skill 範例，**你需要時再複製貼到 `.claude/skills/` 即可**。
...
---
name: explain
description: Use when the user asks "what is this project doing", "白話講一下這個專案", 想要 5 句話講完整個 repo 的目的. Reads PRD and CLAUDE.md, summarizes in plain language for non-engineers.
---
...
`.claude/skills/git/commit.md`（注意要建 `git/` 子資料夾，namespace 用冒號分隔變成 `/git:commit`）：
...
**完整教學見 [`./SUBCLAUDE.md`](./SUBCLAUDE.md)**——包含三大原語對照、何時用、context 隔離、Map-Reduce/Pipeline/Vote 三種彙整模式、安全限制、除錯。

> 初學者**先把單純 skill 寫熟**，等你寫過 5+ 個 skill、開始覺得「這 skill 要跑很久」時，再回去看 SUBCLAUDE.md。
...
3. **不要寫破壞性指令當預設行為**：例如別寫「直接刪掉 .claude/ 重建」，要寫「列出可刪選項等使用者確認」
...
- 確認資料夾結構：`.claude/skills/<name>/SKILL.md` 或 `.claude/skills/<name>.md`
...
- [Claude Code Agent Skills 官方文件](https://antigravity.google/docs/skills)
- [Building Custom Skills in Google Claude Code（Medium）](https://medium.com/google-cloud/tutorial-getting-started-with-antigravity-skills-864041811e0d)
- [Claude Code Hands-On Guide（DEV）](https://dev.to/arindam_1729/antigravity-cli-a-hands-on-guide-to-googles-terminal-coding-agent-5bc7)
