# Claude Code 操作手冊（給 循環工程 學員）

> 這份文件是給「已經會用 AI Studio / Claude Code，現在想轉到 Claude Code」的學員。
> 讀完你會知道：每個檔案在做什麼、什麼時候該打開哪個工具、怎麼讓 AI 變成你的長期協作者。
>
> **如果你還沒裝 Claude Code**，先回去看 [`README.md`](../README.md) 三步驟。

---

## 0. 先講背景：為什麼是 Claude Code 不是 Gemini CLI？

2026 年 5 月 19 日 Google I/O 宣布把 **Gemini CLI 統一到 Claude Code 平台**：CLI 命令從 `gemini` 改為 `claude`、用 Go 重寫（啟動毫秒級、佔用幾 MB RAM）、與 Claude Code共用 agent runtime。Gemini CLI 個人版（AI Pro / Ultra / 免費 Code Assist 帳號）將於 **6 月 18 日停止服務**。企業版 Gemini Code Assist Standard / Enterprise 不受影響。
...
Claude Code / AI Studio 是「點選式」工具，畫面長什麼樣 AI 就吃什麼。Claude Code 不一樣——**它是純文字介面，所有規則、工具、記憶都靠檔案配置**。
...
這份手冊用 **harness engineering**（協作環境工程）的角度，告訴你怎麼把 Claude Code 設定成「**會記得你、會自己用對工具、會跟著你的工作流**」的長期搭檔。
...
你的專案資料夾/
│
├── CLAUDE.md ◄──────── AI 啟動時自動讀，叫做「站立規則」（跨工具新興共通格式）
│
├── docs/
...
└── .claude/
    ...
    ├── SUBCLAUDE.md    ◄── 平行任務分派（2026 殺手特性，大型任務用）
...
- `CLAUDE.md` + `settings.json` = AI 的「身份證 + 出生環境」（每次都會讀）
- `MCP / Skill` = AI 的「外掛能力 + 知識 / 快捷指令」（按需啟用）
- `rules/ + prompts/` = 給人類看的書（怎麼問問題、AI 卡關時該翻哪頁）

> **Claude Code vs Gemini CLI 的差別**：Claude Code 把 Gemini CLI 的 `commands/.toml` 與 `skills/SKILL.md` 兩個原語**合併成單一 Skill 原語**。同一份 markdown 既能讓 AI 自動觸發、也能讓你打 `/skill-name` 手動觸發。比 Gemini CLI 簡單。
...
每次你打 `claude` 啟動 CLI 時，它會**自動掃這些位置**，依序合併成 system instruction：

| 階層         | 路徑                                    | 適合放                |
| ---------- | ------------------------------------- | ------------------ |
| Global     | `~/.gemini/antigravity-cli/CLAUDE.md` | 個人偏好（語言、tone、習慣套件）|
| Project    | `<repo>/CLAUDE.md`                    | 專案規範（角色、技術棧、禁止項）  |
| Subdir     | `<repo>/src/CLAUDE.md`                | 模組級別 invariants    |

> 過渡期說明：全域目錄沿用 `~/.gemini/antigravity-cli/`（Google 為了讓 `agy plugin import gemini` 一鍵搬遷舊有 `~/.gemini/`）。專案目錄改用 `CLAUDE.md`（**Claude Code 與 OpenAI Codex 原生讀**；Claude Code 主吃 `CLAUDE.md`、Cursor 主吃 `.cursor/rules/`，要覆蓋這兩家需另寫 rules 檔指向 `CLAUDE.md`）。
...
會列出當下實際載入的合併內容。如果你剛改了 `CLAUDE.md`，但 `/memory show` 還是舊的，跑：
...
**進階技巧**：`CLAUDE.md` 內可以用 `@path/to/file.md` 把其他檔案內嵌進來，例如：

```markdown
請遵守以下細則：
@.claude/rules/01-keep-it-simple.md
@.claude/rules/02-coding-style.md
```
...
### 3.2 基礎建設：`.claude/settings.json`

這是「**Claude Code 本身的行為**」的設定檔（不是給 AI 看的，是給 CLI 程式看的）。最關鍵的幾個欄位：
...
| `mcpServers` | MCP 工具清單 | 預設空 `{}`，要啟用就從 `.claude/MCP.md` 複製整段貼進去 |
...
你想擴充 AI 的能力
   ...
   │       │
   │       └─ Yes → 用 MCP        （詳見 .claude/MCP.md）
   │
   ├─ 是「自己會判斷該不該做」或「我打一句就要跑」的流程嗎？
   │       │
   │       └─ Yes → 用 Skill      （詳見 .claude/SKILLS.md）
   │
   └─ 是「一條線跑會 context 爆 / 跑太久」的大型任務嗎？
           │
           └─ Yes → 派 Subagents  （詳見 .claude/SUBCLAUDE.md）
```
...
## 4. 循環工程 Sprint 怎麼跟 CLI 接軌

`CLAUDE.md` 規定的 循環工程 十站，在 CLI 裡每一站都有對應的工具 / 技巧讓你跑得更順：
...
確認 `.claude/settings.json` 內 `checkpointing.enabled: true`。TDD 的 REFACTOR 步驟改壞了，打 `/restore` 選快照回滾——比 `git reset` 安全（不動 git 歷史），測試還在就能立刻重跑確認。
...
詳見 [`.claude/MCP.md`](../.claude/MCP.md) 的 playwright 段落。
...
| `/memory show` | 看 AI 現在到底讀到什麼 | debug CLAUDE.md 沒生效時必用 |
| `/memory refresh` | 強制重讀 CLAUDE.md | 改完規則要立刻生效時 |
...
| `/<skill-name>` | 手動觸發任何 skill | `/sec-scan`、`/explain-code`（本模板附 2 個） |
...
- 檢查你是不是在錯的資料夾啟動 CLI（不是 repo root 就掃不到 project CLAUDE.md）
...
- `connected` → 沒事
- `disconnected` → `npx` 套件可能裝失敗。手動跑一次 `.claude/settings.json` 內的 `command + args` 看錯誤訊息
...
- [ ] 把 `CLAUDE.md` 當「給 AI 的合約」而不是文件——它每次都會讀
...
- [ ] 不要把所有規則塞進 `CLAUDE.md`，**拆到 `rules/` 用 `@import` 引用**
...
- [Transitioning Gemini CLI to Claude Code（Google Developers Blog, 2026-05-19）](https://developers.googleblog.com/an-important-update-transitioning-gemini-cli-to-antigravity-cli/)
- [Migrating from Gemini CLI（官方 migration 指南）](https://antigravity.google/docs/gcli-migration)
- [Claude Code Skills 官方文件](https://antigravity.google/docs/skills)

**社群入門**

- [Getting started with Claude Code（Rich Rose, Google Cloud Community）](https://medium.com/google-cloud/getting-started-with-antigravity-cli-3565d5db1e92)
- [Claude Code: A Hands-On Guide（DEV Community）](https://dev.to/arindam_1729/antigravity-cli-a-hands-on-guide-to-googles-terminal-coding-agent-5bc7)
- [Claude Code Deep Dive（agentpedia, 2026-05）](https://agentpedia.codes/blog/antigravity-cli-deep-dive)
- [How to Build Custom Skills in Google Claude Code（Medium）](https://medium.com/google-cloud/tutorial-getting-started-with-antigravity-skills-864041811e0d)
...
1. **跑 `claude` 完成首次啟動三步驟**（配色、條款、資料夾信任）
2. **跑 `/memory show`** 確認你的 `CLAUDE.md` 真的被載入
3. **打開 [`.claude/MCP.md`](../.claude/MCP.md)**，啟用「初學者四件套」（filesystem + fetch + context7 + playwright）
4. **照 [`.claude/SKILLS.md`](../.claude/SKILLS.md)** 寫你的第一個自訂 Skill
5. **直接試打** `/sec-scan`、`/explain-code` 兩個本模板附的 skill
6. **遇到大型任務再讀 [`.claude/SUBCLAUDE.md`](../.claude/SUBCLAUDE.md)**（MVP 階段先跳過）
